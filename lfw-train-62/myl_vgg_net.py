...
def inference(input_images, train=True):
    ...
    with tf.variable_scope('conv1_1') :
        ...
        out_b = tf.nn.local_response_normalization(out_a, bias=2)
        #noise = np.random.normal(0,0.34,[64,224,224,64]).astype('float32')
        #out_c = tf.nn.relu(tf.add(out_b,noise))
    ...
    with tf.variable_scope('fc8') :
        fc3w = tf.get_variable('weights', shape=[4096,62], dtype=tf.float32, initializer=tf.random_normal_initializer(stddev=1e-2), trainable=True)
        fc3b = tf.get_variable('biases', shape=[62], dtype=tf.float32, initializer=tf.constant_initializer(0.0), trainable=True)
